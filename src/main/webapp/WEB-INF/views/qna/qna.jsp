	...
	<table style = "margin: auto;" border = "1" width = "1000" id = "qnalist">
		...
		<tbody id = "insertbody">
         ...
         value = "${vo.regdate}" /> --%>
					...
					<c:choose>
					<c:when test="${vo.comments =='답변완료'}">
					<td><a href="javascript:comm('${vo.qnum }')">${vo.comments }</a></td>
					</c:when>
					<c:otherwise>
					<td><a>${vo.comments }</a></td>
					</c:otherwise>
					</c:choose>
...
/*  		$.ajax({
	...
	function comm(qnum){
		pop = window.open("<c:url value='/qna/qPopup?qnum="+qnum+"' />", "pop","width=570,height=420, scrollbars=yes, resizable=yes"); 
